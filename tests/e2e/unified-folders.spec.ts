...
const summary = (accountId: string, uid: number, subject: string, unread: boolean, starred: boolean): MessageSummary => ({
...
test("shows cached cross-account folders with attribution, shared regex search, and stable selection", async () => {
  ...
  const alphaMessage = summary("alpha", 1, "Alpha unread inbox", true, false);
  ...
  await application.evaluate(({ BrowserWindow, ipcMain }, fixture) => {
    ...
    ipcMain.handle("mail:messages", (_event, accountId: string) => accountId === "alpha" ? [fixture.alphaMessage] : [fixture.bravoMessage]);
    ipcMain.handle("mail:unified-messages", (_event, folder: "inbox" | "starred" | "unread") => {
      ...
      inboxReads += 1;
      return inboxReads % 2 ? [fixture.alphaMessage, fixture.bravoMessage] : [fixture.bravoMessage, fixture.alphaMessage];
    });
    ipcMain.handle("mail:message", (_event, accountId: string) => {
      const message = accountId === "alpha" ? fixture.alphaMessage : fixture.bravoMessage;
      return {
        ...message,
  ...
  }, { bootstrap, alpha, bravo, alphaMessage, bravoMessage });
  ...
  await expect(list.locator(".message-row__account")).toContainText(["Alpha Account · alpha@example.test", "Bravo Account · bravo@example.test"]);
