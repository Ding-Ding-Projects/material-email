...
describe("changelog date model", () => {
  ...
  it("composes text and date predicates and serializes the selection", () => {
    ...
    const selected = filterChangelogEntries(entries, "Beta", text => text.includes("Beta"), "2026-08-01", "2026-08-02");
    expect(selected).toHaveLength(1);
    expect(changelogMarkdown(selected, "Beta", "2026-08-01", "2026-08-02")).toContain("## 2 — Beta");
  });
});
