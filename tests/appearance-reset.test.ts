...
const preferences: Preferences = { language: "bilingual", funnyEnglish: 5, funnyCantonese: 4, theme: "dark", density: "relaxed", accent: "#123456", fontFamily: "Arial", fontScale: 1.5, fontWeight: 700, dimSumEnabled: false, narratorEnabled: true, narratorLanguage: "yue", nativeNotificationsEnabled: true, selectedAccountId: "acct", selectedFolderPath: "Inbox" };
...
describe("appearance reset", () => {
  it("resets only the six appearance fields", () => {
    ...
    expect(reset.language).toBe("bilingual"); expect(reset.funnyEnglish).toBe(5); expect(reset.nativeNotificationsEnabled).toBe(true); expect(reset.selectedAccountId).toBe("acct");
  });
});
