...
- [ ] Auto-detect the signed-in Microsoft account's email/display name from the ID token's claims to prefill the Add Account form (needs `openid`/`profile`/`email` scopes and a non-secret claim surfaced on `OAuthSignInSnapshot`)
