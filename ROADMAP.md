...
- [ ] Exercise every user-visible path in a packaged Electron session (a real `electron-builder --linux dir` packaged build now backs one, with `app.isPackaged === true` and `app.getAppPath()` resolving inside `app.asar` proven against the actual packaged binary, not the dev-mode source-tree launch every other spec in this suite uses; that session drives Mail search/regex/reader, Compose open and close, Drafts, Outbox, all three Contacts sub-tabs, the Calendar new-event dialog, Tasks, eight Settings cards including Identities/Message Filters/PIM Providers/OAuth Vault, the per-tab appearance editor, the Changelog and History date pickers, Notification Centre, the Tools regex playground, and the command palette, with 18 real screenshots and 4 / 4 real-Electron scenarios in `tests/e2e/packaged-build-tour.spec.ts`; still not driven in a packaged session: live account add/OAuth sign-in, PIM record create/edit/delete submission, attachment/quarantine review, POP3/TLS diagnostics, and the startup dim-sum surprise)
...
- [ ] Verify all language modes and every English/Cantonese humor level without changing factual content across every remaining app surface (the focused appearance/date-picker/notification matrix, plus a new packaged-Electron English-1/Cantonese-5-and-reverse extremes proof that the Settings Message Filters status line and the mail Quick Filter's facet labels and invalid-pattern alert switch voice without moving the underlying filter count, are verified; most other surfaces, including Contacts/Calendar/Tasks record content, Compose, and Quarantine, remain unverified)
- [ ] Verify plain-text and regex search from every search surface, including nested settings/adjustment surfaces and invalid, Unicode, multiline, capture, zero-width, and adversarial cases (global Settings, Notification Centre, all six PIM fields, and four Tab Manager discovery persistence/no-match/focus slices plus command-palette and cached-mail/Unified Inbox adversarial, Unicode-zero-width, multiline, and no-match slices are verified; a real packaged Electron session now additionally verifies the nested Settings Message Filters search/regex builder — created through its own real editor form — and the standalone Tools regex playground against nested-quantifier/overlapping-alternation rejection, invalid syntax, Unicode zero-width match-by-code-point, multiline anchors, and capture groups; the Changelog/History text search fields, bulk-tab-close review, and the mail Quick Filter's own adversarial/Unicode/capture depth remain open)
