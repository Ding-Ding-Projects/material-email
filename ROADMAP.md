...
- [x] Run the consolidated type, unit, integration, and build verification (`npm run check`: 59 files / 322 tests on the current tree; focused Settings search passed 1 / 1 real-Electron scenario, cached-mail search passed 3 / 3, forced-colors/focus passed 2 / 2, focused Outbox redaction/recovery passed 1 / 1, responsive/accessibility passed 2 / 2, Windows window controls passed 3 / 3, and broader evidence remains listed in `HANDOFF.md`)
...
- [ ] Verify plain-text and regex search from every search surface, including nested settings/adjustment surfaces and invalid, Unicode, multiline, capture, zero-width, and adversarial cases (the global Settings persistence/no-match/focus slice, command-palette slice, and cached-mail/Unified Inbox adversarial, Unicode-zero-width, multiline, and no-match slice are verified)
