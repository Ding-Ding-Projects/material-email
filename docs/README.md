# Documentation index

Material Email documentation is organized by feature area. Every article records behavior, configuration, failure modes, security considerations, verification evidence, current status, and suggested next reading.

## Architecture

- [Category index](architecture/README.md)
- [Windows Electron foundation](architecture/windows-electron-foundation.md)
- [Security boundaries](architecture/security-boundaries.md)
- [LibreOffice source-reference map](architecture/libreoffice-source-map.md)

## Mail

- [Category index](mail/README.md)
- [Accounts and connectivity](mail/accounts-and-connectivity.md)
- [Synchronization and folders](mail/synchronization-and-folders.md)
- [Local unified folders](mail/unified-folders.md)
- [Cached conversation grouping](mail/conversation-grouping.md)
- [Reading and message safety](mail/reading-and-message-safety.md)
- [Compose, drafts, and sending](mail/compose-drafts-and-sending.md)

## Experience

- [Category index](experience/README.md)
- [Material interface and accessibility](experience/material-interface-and-accessibility.md)
- [Language and humor controls](experience/language-and-humor.md)
- [Search and regex builder](experience/search-and-regex-builder.md)
- [Tabs and discovery](experience/tabs-and-discovery.md)
- [Notifications](experience/notifications.md)
- [Appearance customization](experience/appearance-customization.md)

## Data and tools

- [Category index](data/README.md)
- [Local state and history](data/local-state-and-history.md)
- [Local-history deletion evidence](data/local-history-deletion-evidence.md)
- [External editor and export](data/external-editor-and-export.md)
- [Changelog](data/changelog.md)

## Personal information management

- [Category index](pim/README.md)
- [Contacts, mailing lists, and vCard](pim/contacts-mailing-lists-and-vcard.md)
- [Calendars and events](pim/calendars-and-events.md)
- [Tasks and refresh ordering](pim/tasks-and-refresh-ordering.md)
- [PIM persistence and transaction history](pim/persistence-and-transactions.md)

## Delivery

- [Category index](delivery/README.md)
- [Development and packaging](delivery/development-and-packaging.md)
- [Verification matrix](delivery/verification.md)

## Documentation status

These articles describe the development tree, not a shipped release. Anything without direct verification is marked **Foundation** or **Open**. The local site at [`../site/index.html`](../site/index.html) presents the same inventory in an interactive tabbed view.
