...
| Electron end-to-end | `npm run test:e2e` (Playwright over `tests/e2e`) | 29 spec files are present, including `packaged-build-tour.spec.ts`, which is the first spec in this suite to launch a genuinely *packaged* Electron build (`electron-builder --linux dir`) rather than `electron .` against the source tree; re-run in this pass under `xvfb-run`: 4 / 4 scenarios passed, proving `app.isPackaged === true`, `MATERIAL_EMAIL_DEV_URL` ignored when packaged, 18 real screenshots across the major workspace surfaces, and new adversarial/Unicode/bilingual proof for the Message Filters and Tools regex surfaces. The release workflow never invokes this suite. | Run the full suite in CI; a Windows NSIS-packaged session (Linux `dir`-target packaging is not Windows installer proof); clean VM, screen reader, and full display-scale matrix |
