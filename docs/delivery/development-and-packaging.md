...
`npm run verify:package` requires exactly one top-level x64 NSIS installer with the versioned artifact name, a minimum size, valid DOS and PE structure, and the explicit retained-data policy; it reports the SHA-256 digest. DOS/PE structure is not an Authenticode signature check. `npm run verify:installer` silently installs that exact file into a temporary directory and requires the candidate's first packaged smoke launch to begin with an absent isolated user-data directory. The smoke result must identify that override, report first-run state, and match the deterministic application defaults for language, independent humor levels, appearance, notification/narrator settings, retention policy, and loaded window state.

The same verifier then writes a bounded synthetic settings/window fixture into that isolated profile, launches the packaged candidate again, and rejects any difference in the reported settings or loaded window state. It hashes the settings and window-state files, silently uninstalls, proves the executable is gone, and requires both files plus the existing random retention probe to remain unchanged after uninstall. The JSON report always records `cleanMachine: false`, `defaultWindowsProfile: false`, `interactiveFirstLaunch: false`, and `authenticodeSignatureChecked: false`; the harness cannot promote its temporary-profile smoke evidence into those stronger claims.

Passing `--baseline <prior-installer.exe>` changes the verifier into a strict upgrade run. It validates and hashes both PE files, refuses an equal-version reinstall or downgrade, installs and launches the baseline, seeds the synthetic settings/window fixture, installs the candidate into the same isolated install directory, and checks the fixture hashes before candidate launch. The candidate is also launched once against a separate absent isolated profile to check current defaults, then against the retained upgrade profile to check the seeded settings/window values. Upgrade and uninstall must preserve the random probe; settings and window files must additionally survive uninstall. `--candidate` and `--metadata` permit an explicitly built candidate outside the repository output directory. The JSON report identifies both artifact hashes, both smoke-reported versions, installed-executable hashes, the Windows version, state hashes, and every evidence limitation.
...
The new upgrade path was exercised locally on Windows 10.0.26200 using published baseline `0.19.1` and a disposable `0.999.1` candidate built from source commit `22129fc`. The verifier recorded baseline installer SHA-256 `22588b7f326a0fe0d4e9542f99debdf927bcd7764186752d80ac5472836cae06`, candidate SHA-256 `afae75f001fdbacc3a6b4f87a60408939fbdd3dc7b82014dd15f0e30ee3d08a6`, correct baseline and candidate smoke versions, same-directory replacement, candidate removal on uninstall, and the unchanged probe SHA-256 `3b22cb8daa3037fd87493b3e4fe24a1e0669ad71194115c87547583fc834d3de` after upgrade and uninstall. This remains genuine historical local upgrade evidence, but it predates the new settings/window fixture and the host was neither clean nor a default-profile disposable VM. The current `npm run check` passes 56 test files / 308 tests and the 22-module renderer production build.

The release workflow resolves exactly one Windows installer from the latest prior release, retries its download, verifies the API-reported size, and requires the baseline-to-candidate versions, hashes, same-directory upgrade, absent-isolated-profile defaults, retained settings/window state, and random-probe evidence before publishing. Its first-release path remains explicitly labeled as a single-installer lifecycle rather than an upgrade. Focused contract coverage passes 7 / 7 tests for version/name/argument/PE guards plus deterministic profile fixture, smoke, and stronger-claim refusal behavior. This source evidence does not itself claim a hosted run, clean-machine/default Windows-profile test, interactive first launch, Authenticode verification, approved application icon, or Pages deployment.
