...
**Implemented foundation with bounded local preflight, opt-in live TLS inspection, an ephemeral OAuth authorization-code/PKCE boundary, and a mock-only token-lifecycle harness.** Manual configuration, provider presets, standards-based DNS discovery, deterministic host/port/security diagnostics, a credential-free certificate inspection action, and a main-process OAuth authorization state machine exist. A separate test/demo module models exchange, refresh, expiry, and revocation over ephemeral encrypted storage. No provider client registration or production token exchange/storage ships in this build, so provider interoperability and connected OAuth accounts remain open.
...
`EphemeralAesGcmOAuthTokenStorage` satisfies the encrypted-storage contract by keeping only AES-256-GCM ciphertext in a process-local map, binding ciphertext to its storage key with authenticated additional data, replacing/zeroing old blobs, and zeroing its random 256-bit key on disposal. It is deliberately non-durable and is **not** Windows credential storage or production at-rest evidence. `ProductionEncryptedOAuthTokenStorageStub` stores nothing: without a reviewed provider registration it returns `provider-registration-required`; even with registration it returns `production-storage-adapter-required` until a reviewed Windows-backed adapter exists.
...
Discovery performs DNS lookups only; it does not fetch or execute provider-hosted configuration code. Results remain advisory and are shown for review before credentials are used. The preflight runs locally and never logs or transmits entered settings; the main process repeats the blocking check so renderer bypass cannot start a transport. Live inspection is a separate explicit action with a credential-free contract, bounded server replies, sanitized failure copy, redacted renderer result, and no persistence. OAuth authorization secrets are confined to the main-process attempt, zeroed/cleared at terminal cleanup, omitted from IPC, and never passed to JSON/history stores or logging calls. The mock lifecycle has no filesystem, network, IPC, or logging route; its ephemeral ciphertext is destroyed on disposal. Never log secrets or server-returned private data. Plain transport should be treated as unsafe. Provider presets, future authorization/token endpoints and scopes, redirect policy, Windows encryption, rotation, and revocation need regular security review without silently changing an existing account.

## Verification

Focused OAuth authorization verification passes 8 state-machine/PKCE/callback unit tests and 1 real-Electron scenario covering bilingual semantics, no pasted-token field, disabled account actions, waiting/cancel states, focus preservation through polling, and the 760 × 560 width bound. The mock-only lifecycle passes 7 focused unit tests covering ciphertext-at-rest inspection, exchange, expiry, refresh rotation, revocation, revoke-failure deletion, invalid inputs/transitions, demo construction, and both production refusal stages. The combined IPC/security focus is rerun with a static proof that production main/preload/AppService do not import the mock module. Focused local-preflight verification passes 2 unit files / 8 tests plus 1 real-Electron scenario. Focused live-inspection verification passes 2 unit/IPC files / 15 tests plus 1 real-Electron loopback-TLS scenario. Live Gmail, Outlook, Yahoo, iCloud, custom-domain authorization, real token exchange/refresh/revocation, provider scopes/consent, durable Windows token storage, public-provider callback variations, and provider interoperability remain untested and unclaimed.
