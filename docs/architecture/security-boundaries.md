...
The current local test gate passes 48 files / 239 tests. Coverage includes bounded MIME fetches and parser output, malformed-header and unterminated-multipart cases, active HTML removal, default-deny and exact-origin remote-image handling, persisted allow/revoke state, authentication of every IPC channel's sender/frame/location, trusted-location gating for mailto delivery, bounded runtime payload validation, attachment/editor path authorization, serialized state writes, corruption recovery, snapshot validation, guarded local-revision retention, read-only deletion evidence, and the Windows OAuth vault registration/encryption/rotation/revoke-clear boundary. The focused OAuth real-Electron file passes 2 / 2 scenarios. No penetration test, broad malformed-message or parser wall-time matrix, malicious attachment-content matrix, live image-server or certificate-failure matrix, live-provider OAuth audit/interoperability, OpenPGP/S/MIME review, cryptographic-erasure proof, or clean-machine privacy audit has been completed.
