...
**Core matcher, command-palette, cached-mail adversarial integration, global Settings, and four Tab Manager discovery restart/focus tests pass; the complete every-surface and nested-settings matrix remains open.** JavaScript regex evaluation is local and bounded, but not isolated by a hard timeout.
