...
Settings, semantic and whole-workspace History, Notifications, the six PIM scopes, the four Tab Manager discovery scopes, Tab Manager bulk close, and the command palette all use this shared field-bound renderer path. Global Settings, Notification Centre, each PIM field, each Tab Manager discovery field, and bulk close intentionally restore their bounded mode, pattern, and normalized flags from renderer-local storage; samples and builder-open state reset at restart. The PIM fields are Contacts, Mailing Lists, Calendar Events, Tasks, PIM Transaction History, and the mailing-list member picker; each has independent state. Bulk close additionally restores its direction and explicit pinned-inclusion choice, discloses both beside the controls and in its semantic preview, and treats whitespace-only input as empty before destructive review. A valid Settings, Notification Centre, PIM, or tab-discovery query with no result produces a named polite status, independently humor-levelled English/Cantonese guidance, and a keyboard-reachable action that returns focus to its field. PIM collection surfaces also expose factual matching/total counts and a distinct named invalid-pattern alert without changing records. Inside the Tab Manager and mailing-list editor, nested builders are collision-contained so their controls remain pointer-operable; close and <kbd>Escape</kbd> return focus without dismissing the owning manager or editor.
...
- Notification Centre search storage: `material-email.notification-search.v1`, containing only bounded `mode`, `pattern`, and normalized `flags`
- PIM search storage: `material-email.pim-searches.v1`, containing those same bounded fields independently for Contacts, Mailing Lists, Calendar Events, Tasks, PIM Transaction History, and the mailing-list member picker
...
Patterns and samples remain local. Global Settings, Notification Centre, the six PIM fields, the four Tab Manager discovery fields, and bulk close treat use as intent to restore their bounded mode, pattern, and flags on this computer and disclose that behavior in the attached builder; samples are never serialized. PIM search changes only filtering: the service CRUD, membership, transaction, import/export, and restore semantics are unchanged. Bulk close also persists and visibly restates direction and pinned inclusion, but it never derives candidates from an empty, whitespace-only, invalid, or rejected risky pattern. Other search surfaces keep their documented persistence boundaries. Keep bounds and risky-pattern checks in shared logic. For stronger guarantees, evaluate regex in a worker that can be terminated by a deadline or adopt a linear-time engine with documented dialect differences.
...
The focused Notification Centre parser/localization slice passes 2 files / 19 tests. It covers strict bounded restoration, normalized flags, omission of sample/open/unrelated state, five-level no-match guidance, and factual localized counts. A dedicated 1 / 1 real-Electron restart scenario proves input/builder synchronization, independent Notification-only storage, regex restoration, sample reset, distinct named invalid/no-match states, inverse bilingual levels, and edit/<kbd>Escape</kbd> focus return. The existing 1 / 1 action scenario separately confirms read/dismiss and structured notification actions remain intact.

The focused PIM-search slice passes 1 renderer file / 5 tests. It covers the exact six-key allowlist, independent mode/pattern/flag round trips, corrupt or partial storage, bounds, unsupported-flag removal, omission of samples/open/unrelated state, four semantic result states, exact counts, and independent English/Cantonese humor levels. A dedicated 1 / 1 real-Electron scenario creates real local PIM records and proves all six fields persist independently across restart, samples reset, invalid regex stays distinct from valid no-match, localized counts retain exact facts, recovery actions return focus, and <kbd>Escape</kbd> closes the member picker builder without closing its editor. The filtered member list now fingerprints the authoritative selected UID set for dirty-state checks, so hiding a selected member does not invent an unsaved membership change.
