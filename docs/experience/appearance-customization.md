...
The completion target is an anchored, non-modal editor available from every rendered element, with keyboard access, focus return, per-property reset, per-element reset, global reset, presets, and theme export/import.
