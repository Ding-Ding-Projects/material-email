...
The following remain explicitly **open or not fully verified**: live OAuth provider registration, real token exchange, Windows-backed token storage/refresh/revocation and connected OAuth accounts; POP; server-complete all-account synchronization/threading; native Windows notifications; SQLite mail indexing; remote CardDAV/CalDAV/task-provider synchronization; broad vCard/ICS interoperability; recurrence expansion and alarm delivery; cryptographic message features; antivirus/content scanning; clean-machine installer proof; CI releases; GitHub Pages; and live-site publication. The ephemeral OAuth PKCE/loopback state machine and mock-only encrypted exchange/refresh/revoke harness, cached local unified folders and bounded conversation grouping, autoconfiguration, queued mail actions/outbox, attachment saving/quarantine, Git-backed snapshots, and the local organizer pass focused local checks. Mock tokens are never provider evidence; the documented provider, scale, screen-reader, and packaging matrices remain open.
