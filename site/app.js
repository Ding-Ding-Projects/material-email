...
const ARTICLES = [
  ...
  article("reader","Reading, MIME, and attachments","閱讀、MIME 同附件","verified","MIME parsing, default-deny per-message remote-image consent, isolated reader markup, and safe save paths.","../docs/mail/reading-and-message-safety.md","Raw MIME becomes headers, text, two sanitized HTML variants, normalized image origins, flags, and attachment metadata.|The default document has no images; explicit per-message consent admits only its listed HTTP(S) image origins and can be revoked.|Complex formatting disappears, remote loads can disclose open timing, and saved files receive no antivirus analysis yet.|The app CSP is unchanged; the opaque message frame keeps scripts, forms, frames, connections, referrers, and same-origin access denied.|Focused unit and real-Electron tests prove default deny, source summary, persisted restart, revocation, focus, and bilingual semantics; certificate UX and live image servers are open.","security,sync,compose"),
