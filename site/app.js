...
const ARTICLES = [
  ...
  article("reader","Reading, MIME, and attachments","閱讀、MIME 同附件","verified","Bounded MIME parsing, default-deny per-message remote-image consent, isolated reader markup, and safe save paths.","../docs/mail/reading-and-message-safety.md","Raw MIME reads stop at 32 MiB plus one detection byte; smaller header, body, attachment-count, and decoded-attachment ceilings apply before caching.|The default document has no images; explicit per-message consent admits only its listed HTTP(S) image origins and can be revoked.|Legitimate oversized messages are refused; broad malformed corpora, parser wall-time isolation, provider interoperability, and antivirus analysis remain open.|MIME refusal leaves cache/server state unchanged; the app CSP and opaque frame still deny scripts, forms, frames, connections, referrers, and same-origin access.|Focused tests cover bounded ranges, every declared ceiling, stable failures, NUL headers, attachment fan-out, and unterminated multipart while existing sanitizer/real-Electron consent evidence remains green.","security,sync,compose"),
  ...
  article("accessibility","Material interface and accessibility","Material 介面同無障礙","open","M3 shell, semantic tabs, keyboard focus preservation, live regions, and reduced motion.","../docs/experience/material-interface-and-accessibility.md","Mail, Settings, Changelog, History, Notifications, and Tools are discrete panels.|Theme, density, accent, and fonts persist; layouts must survive 100–200% scaling.|Dynamic focus, bilingual clipping, title bars, popovers, contrast, and targets can fail.|Message markup never enters app chrome; accessible names must protect private content.|A focused real-Electron test proves primary-tab Arrow/Home/End focus and tab/tabpanel semantics; native screen-reader and display-scale matrices remain open.","tabs,appearance,language"),
  ...
  article("regex","Search and regex builder","搜尋同正規表達式建立器","verified","Plain-text-first search with a field-bound JavaScript RegExp builder.","../docs/experience/search-and-regex-builder.md","Builder synchronizes pattern, flags, validation, sample, matches, captures, copy, export; command-palette search now uses the same independent field-bound model.|Limits are 2,048 pattern characters, 50,000 sample characters, 200 matches, flags i/m/s/u.|Risk detection is heuristic, not a timeout; JavaScript differs from PCRE and .NET.|Patterns and samples stay local; invalid or empty patterns cannot run palette commands or drive bulk actions.|Focused tests cover literals, bilingual command labels, case, captures, zero-width progress, invalid syntax, nesting, flags, physical palette anchoring, nested Escape, and Enter activation; the complete every-surface matrix remains open.","tabs,history,changelog"),
  article("tabs","Tabs and discovery","分頁同探索","foundation","Browser-style tabs with order, pins, groups, searches, close preview, styling, and verified roving focus.","../docs/experience/tabs-and-discovery.md","Pinned tabs lead ordinary tabs; the app model includes four search scopes and inverse close.|Order, closed state, pins, groups, collapse, and appearance persist locally.|Active close, drag/keyboard parity, collapsed reveal, and unsaved compose can fail.|Search uses visible labels; pinned and unsaved tabs stay protected by default.|A focused real-Electron test verifies Arrow/Home/End focus, one roving tab stop, reciprocal panels, and a visible inset focus rule; complete grouping, overflow, assistive-technology, and scaling checks are open.","regex,appearance,accessibility"),
...
const validateReleaseMetadata = (value) => {
  ...
  const hasCodeName=value.codeName!==null,hasPhoto=value.photoFile!==null;
  if(hasCodeName!==hasPhoto)throw new Error("Published release code name and photo must both be present or both be null.");
  if(hasCodeName){const dish=DIM_SUM.find((item)=>item.file===value.photoFile);if(typeof value.codeName!=="string"||!dish||value.codeName!==dish.en+" · "+dish.yue)throw new Error("Published release dish metadata is invalid.");}
...
const releaseAwareArticle = (item) => {
  ...
  if(item.id==="changelog")return {...item,status:"foundation",summary:"The published release is identified here while the complete searchable in-app history remains open.",facts:[item.facts[0],item.facts[1],item.facts[2],item.facts[3],"This Pages build records "+releaseMetadata.version+" and "+releaseMetadata.releaseDate+(releaseMetadata.codeName?", with code name "+releaseMetadata.codeName:"; the verified dish catalog was exhausted, so no code name or photo was reused")+"; the complete in-app history and advanced picker remain open."]};
  if(item.id==="packaging")return {...current,status:"released",summary:"A verified Windows release is published with its installer"+(releaseMetadata.codeName?" and local catalog photo.":"; catalog exhaustion is recorded without reusing a dish."),facts:[current.facts[0],current.facts[1],"The published installer is still unsigned; clean-machine, upgrade, provider, and enterprise-policy matrices remain open.",current.facts[3],"Release "+releaseMetadata.version+" was published on "+releaseMetadata.releaseDate+" after installer lifecycle and downloaded-asset verification"+(releaseMetadata.codeName?"; code name "+releaseMetadata.codeName+".":"; no code name or photo was assigned because every verified catalog dish was already used.")]};
  return current;
};
...
const applyReleasePresentation = () => {
  ...
  if(supporting)supporting.textContent=tx("Published ","已發布 ")+releaseMetadata.releaseDate+" · "+(releaseMetadata.codeName||tx("No code name assigned","未有分配版本代號"));
};
...
const releasedHomeHtml = () => {
  const counts=(value)=>allArticles().filter((item)=>item.status===value).length,dish=DIM_SUM.find((item)=>item.file===releaseMetadata.photoFile),decorated=Boolean(dish&&releaseMetadata.codeName);
  const en=decorated?["The verified Windows release is ready with its evidence attached.","A tested Windows release has arrived with receipts.","The Windows release has landed, installer and tiny steamer basket included.","The installer passed its obstacle course and brought dim sum to celebrate.","Ribbon cut: Windows installer verified, release shipped, har gow reporting for duty."]:["The verified Windows release is ready with its evidence attached.","A tested Windows release has arrived with receipts and no borrowed code name.","The Windows release landed after the dish catalog ran out; the installer did not wait for dessert.","The installer passed its obstacle course while the empty steamer basket filed an honest report.","Ribbon cut: Windows installer verified; all ten dish names were already on duty, so nobody wore a fake moustache."];
  const yue=decorated?["已驗證 Windows release 連證據一齊準備好。","Windows release 測試完成，單據齊全。","Windows release 已到，installer 同小蒸籠一齊登場。","Installer 過晒障礙賽，仲帶埋點心慶祝。","剪綵喇：Windows installer 已驗證，release 出街，蝦餃正式返工。"]:["已驗證 Windows release 連證據一齊準備好。","Windows release 測試完成，冇借人哋個版本代號嚟充撐場面。","點心目錄用晒，Windows release 照樣落地；installer 冇企喺度等甜品。","Installer 過晒障礙賽，空蒸籠就誠實交代冇新點心。","剪綵喇：Windows installer 已驗證；十個點心名都有正職，冇邊個要黐假鬚返多更。"];
  const tone=state.prefs.language==="yue"?yue[state.prefs.funnyCantonese-1]:state.prefs.language==="bilingual"?en[state.prefs.funnyEnglish-1]+" · "+yue[state.prefs.funnyCantonese-1]:en[state.prefs.funnyEnglish-1];
  const cards=[["released",counts("released")],["verified",counts("verified")],["foundation",counts("foundation")],["open",counts("open")]];
  const summary=decorated?'<article class="release-summary"><img src="'+esc(dish.url)+'" alt="'+esc(releaseMetadata.codeName)+'"><div><p class="eyebrow">'+esc(tx("BUILD CODE NAME","版本代號"))+'</p><strong>'+esc(releaseMetadata.codeName)+'</strong><span>v'+esc(releaseMetadata.version)+'</span><time datetime="'+esc(releaseMetadata.releaseDate)+'">'+esc(releaseMetadata.releaseDate)+' UTC</time></div></article>':'<article class="release-summary release-summary--plain"><div><p class="eyebrow">'+esc(tx("CATALOG EXHAUSTED","點心目錄已用完"))+'</p><strong>'+esc(tx("No code name assigned","未有分配版本代號"))+'</strong><span>v'+esc(releaseMetadata.version)+'</span><time datetime="'+esc(releaseMetadata.releaseDate)+'">'+esc(releaseMetadata.releaseDate)+' UTC</time></div></article>';
  const assetNote=decorated?tx("The code-name photo and all ten surprise photos are bundled inside this Pages artifact; no food image is fetched remotely.","版本代號相同十張驚喜相全部內置喺呢個 Pages artifact；唔會遙距下載點心相。"):tx("All ten verified dish names were already used, so this release has no code-name photo. The local surprise catalog remains bundled and no food image is fetched remotely.","十個已驗證點心名都已經用過，所以今個 release 冇版本代號相。驚喜目錄仍然內置，亦唔會遙距下載點心相。");
  return '<section class="page" id="panel-home" role="tabpanel" aria-labelledby="tab-home"><div class="hero"><div><p class="eyebrow">'+esc(tx("PUBLISHED DOCUMENTATION · RELEASE ","已發布文件 · RELEASE ")+releaseMetadata.version)+'</p><h1>Material Email</h1><p>'+esc(tone)+'</p><div class="hero-actions"><a class="button button--filled" target="_blank" rel="noreferrer noopener" href="'+esc(releaseMetadata.releaseUrl)+'">'+esc(tx("View release and installer","查看 release 同 installer"))+'</a><button class="button button--tonal" data-action="page" data-page="docs">'+esc(tx("Read the docs","閱讀文件"))+'</button></div></div>'+summary+'</div><section class="section"><div class="card-grid">'+cards.map(([status,count])=>'<article class="card"><span class="status-chip" data-status="'+status+'">'+count+'</span><h2>'+esc(statusName(status))+'</h2><p>'+esc(status==="released"?tx("Published release evidence is linked above.","上面有已發布 release 證據。") : status==="open"?tx("Explicit gaps remain visible.","缺口清楚列出。"):tx("Evidence is scoped to the claim.","證據只支持相應聲明。"))+'</p></article>').join("")+'</div></section><section class="section"><div class="notice"><strong>'+esc(tx("Local release asset:","本機 release 資產："))+'</strong> '+esc(assetNote)+'</div></section></section>';
};
...
const releasedStatusHtml = () => {
  ...
  return '<section class="page" id="panel-status" role="tabpanel" aria-labelledby="tab-status"><header class="page-header"><p class="eyebrow">'+esc(tx("PUBLISHED RELEASE","已發布 RELEASE"))+'</p><h1>'+esc(tx("Release and verification status","Release 同驗證狀態"))+'</h1><p>'+esc(tx("This published page received metadata only after the Windows release job verified its installer, tag, and optional catalog decoration.","呢個已發布頁面只會喺 Windows release job 驗證 installer、tag 同可選目錄版本裝飾之後收到 metadata。"))+'</p><a class="button button--filled release-link" target="_blank" rel="noreferrer noopener" href="'+esc(releaseMetadata.releaseUrl)+'">'+esc(tx("Open release ","開啟 release ")+releaseMetadata.version)+'</a></header><div class="table-scroll"><table class="matrix"><thead><tr><th>'+esc(tx("Area","範圍"))+'</th><th>'+esc(tx("Evidence now","目前證據"))+'</th><th>'+esc(tx("Still required","仍然需要"))+'</th></tr></thead><tbody>'+rows.map((row)=>'<tr>'+row.map((cell)=>'<td>'+esc(cell)+'</td>').join("")+'</tr>').join("")+'</tbody></table></div></section>';
};
