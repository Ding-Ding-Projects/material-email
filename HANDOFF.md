...
Browser OAuth, POP, unified folders, threading, SQLite mail indexing, cryptographic history erasure/object reclamation and every-record restore, CardDAV/CalDAV/ICS/task providers, broad vCard interoperability, recurrence expansion, alarm delivery, PIM at-rest encryption, message cryptography, antivirus/content scanning, clean-machine lifecycle proof, hosted verification of the new upgrade gate, wiki synchronization, and Pages deployment remain open. Native Windows notifications, attachment risk warnings, persisted local quarantine with explicit release/delete, searchable local revision diff/label/restore, bounded retention/pruning, read-only deletion evidence, the factual in-app changelog, and a locally exercised strict NSIS upgrade verifier are implemented.
